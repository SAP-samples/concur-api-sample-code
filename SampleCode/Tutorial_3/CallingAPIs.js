/** 
 * Tutorial 3 - Calling APIs
 * =========================
 * 
 * Purpose: Adds the calling of Concur APIs to the previous tutorial using a multiple use bearer token.
 * 
 * 
 * Method: Uses the refresh token to get a multiple use bearer token to call a Concur API.
 * 
 * 
 * Required:
 ...
 * - Company Request Token -> Refresh token -> Bearer token -> API Call
 *
 *  
 * Company Request Token -> Refresh token -> Bearer token -> API Call flow:
 ...
 * 
 * 3. Bearer token:
 *  - Generate a bearer token using this app.
 *    - Used to make API calls.
 *    - Token lifetime = 1 hour.
 *    - Multiple use.
 * 
 * 4. API Call:
 *  - Call the api using this app.
 *    - One bearer token per API call.
 * 
 * 
 * Procedure:
 ...
 * 5. If the validity check is successful, the refresh token in the refresh token object is used to make an API call to get a bearer token.
 * 6. If the API call is successful, the bearer token data is converted from JSON format into a bearer token string.
 * 7. The bearer token string is used to make the API call to the Profile from ID v4 endpoint.
 * 8. If the API call is successful, the JSON response is displayed on the console.
 * 
 * 
 * */

/**
 * Creates constants 'savedCredentialsPathFileName' and 'savedRefreshTokenObjectPathFileName' using the 'path' module to determine the absolute paths to the respective files in the current directory (using the '__dirname' variable). The resulting paths are logged to the console. 
 ...
 * If date1 is greater than date2, the function logs the message "Refresh token is valid. Using existing token to make API Calls." to the console and then calls another function named useRefreshGrantToGetBearerToken(). This function is responsible for using the existing token to make API calls.
...
function checkForRefreshTokenDateExpiry(date1, date2) {
    if(date1 > date2){
      ...
      useRefreshGrantToGetBearerToken();
    } else {
...
/**
 * This code defines a function useRefreshGrantToGetBearerToken() that retrieves a Bearer Token using a Refresh Token by making a POST request to us.api.concursolutions.com.
 * The function starts by printing some information to the console using console.log(). Then, it creates a postData variable, which is a URL encoded string containing the relevant client credentials, grant type, and refresh token.
 * Next, it sets some options for the request to us.api.concursolutions.com through an object named options. These options include the host, request method, path, content type, and content length.
 * Subsequent lines of code create a request object named req using the HTTPS request() method that accepts the options object. This creates a request to us.api.concursolutions.com and sets a callback function to gather the response data.
 * The callback function then listens for incoming data through event listeners for 'data' events. Upon reception of all data, these 'data' events concatenate the chunks into an empty string variable result. After concatenating, the 'end' event is triggered, which initiates JSON parsing on result. 
 * If status code retrieval was successful, the function sends console messages and calls another function called callingAPIs() with the newly created bearerToken. If bearerToken is undefined, an error message prints to the console.
 * Lastly, callback events listen for errors in the request and response. Once the request is complete (either success or error), the function sends the request using .write() and .end() functions.
 */
function useRefreshGrantToGetBearerToken() {
  console.log("Using refresh token to get Bearer Token.");
  console.log("refreshTokenObject.refresh_token = " + refreshTokenObject.refresh_token);
  ...
  var req = https.request(options, res => {
    ...
    res.on('end', function () {
      ...
      dataRetrieved = JSON.parse(result);
      // Store this refresh token. Use this refresh token to call APIs.
      bearerToken = dataRetrieved.access_token;
      console.log("bearerToken (access token) = " + bearerToken);
      console.log("This bearer token will be used to authenticate your API calls.");
...
/**
 * This is a JavaScript function that gets a user profile from a third party service API using an authentication token. The function sends an HTTPS GET request using the options object which contains the host, method, path, and headers information. The Authorization header, which includes the bearer token, is used to authenticate the request. 
 * The function sets up an HTTPS request object and sends the request. The response from the API is received in chunks and is concatenated into the result variable. When the entire response is received, the result variable is parsed as JSON and assigned to the dataRetrieved variable. 
 * The processed response data is assigned to the resultUsersProfileAPI variable and logged to the console for the user to see. If there is an error in sending or receiving the request, it is logged in the console.
 */
function getProfileFromIDv4() {
  // Obtain a profile from ID Pv4 Using Authtoken Auth flow
  console.log("Calling Profile from ID v4 API.")
  ...
  var req = https.request(options, res => {
    ...
    res.on('end', function () {
      console.log("Status code = " + res.statusCode)
      dataRetrieved = JSON.parse(result);
      resultUsersProfileAPI = dataRetrieved
      console.log(resultUsersProfileAPI)
    });
    res.on('error', function (err) {
      console.log(err);
    })
